...
async fn main() {
    ...
    let mut initial_stocks = vec![
        Stock::new("V", "Vulyenne"),
    ];
        ...
        loop {
            ...
            let status = match input {
                "buy" => {
                    ...
                    let mpsc_status = server_tx.send(Query::Buy(symbol, Order::new(creator_id, price, quantity))).await;
                ...
                "sell" => {
                    ...
                    let mpsc_status = server_tx.send(Query::Sell(symbol, Order::new(creator_id, price, quantity))).await;
        ...
        loop {
            for stock in stocks.values_mut() {
                ...
                for trade in executed_trades {
                    market_speak(format!("Market says> Trade executed: {:#?}", trade), &mut stdout, false);
                }
            };
                    ...
                    let status = match query {
                        ...
                        Query::OHLC(symbol) => {
                            if let Some(stock) = stocks.get(&symbol) {
                                ...
                                market_tx.send(QueryResponse::OHLC(open, high, low, close)).await
                            } else {
                                market_tx.send(QueryResponse::SymbolNotFound).await
                            }
                        },
                        Query::BuyOrders(symbol) => {
                            if let Some(stock) = stocks.get(&symbol) {
                                market_tx.send(QueryResponse::QueriedOrders(stock.get_buy_orders())).await
                            } else {
                                market_tx.send(QueryResponse::SymbolNotFound).await
                            }
                        },
                        Query::SellOrders(symbol) => {
                            if let Some(stock) = stocks.get(&symbol) {
                                market_tx.send(QueryResponse::QueriedOrders(stock.get_sell_orders())).await
                            } else {
                                market_tx.send(QueryResponse::SymbolNotFound).await
                            }
                        }
...
fn market_speak(message: String, stdout: &mut StandardStream, error: bool) {
    if let Err(e) = stdout.set_color(ColorSpec::new().set_fg(Some(MARKET_OUTPUT_COLOUR))) {
        println!("Could not set terminal color({:#?}). The next statement is probably from the Market:", e);
    }
    ...
    if let Err(e) = stdout.reset() {
        println!("Could not set terminal color({:#?}). Market output ends.", e);
    }
}